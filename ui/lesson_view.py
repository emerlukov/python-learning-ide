...
class LessonView(BoxLayout):
    ...
    def __init__(self, app, lesson, lesson_manager, **kwargs):
    ...
    def _create_ui(self):
        ...
        self.check_btn.bind(on_release=self._check_task)

        # Кнопка отметки о прохождении
        is_completed = self.lesson_manager.is_lesson_completed(lesson_id)
        ...
        buttons_layout.add_widget(self.check_btn)
        ...
        if saved_code and hasattr(self, 'practice_editor'):
            if hasattr(self.practice_editor, 'set_values'):
                # Для InteractiveCodeWidget нужно установить значения полей
                # Пока просто пропускаем, так как шаблон определяет структуру
                pass
    ...
    def _apply_tab_theme(self):
        ...
        for i, tab in enumerate(self.tab_panel.tab_list):
            ...
            # Добавляем правый отступ (кроме последней)
            if i < len(self.tab_panel.tab_list) - 1:
                tab.padding = [dp(5), dp(2), dp(9), dp(2)]  # правый отступ больше
            else:
                tab.padding = [dp(5), dp(2), dp(5), dp(2)]

        # Устанавливаем цвет для активной вкладки
        if self.tab_panel.current_tab:
            self.tab_panel.current_tab.background_color = tab_active_bg

        # Фон всей панели вкладок
        ...
        # Закрашиваем область заголовков (header)
        if hasattr(self.tab_panel, '_tab_header'):
            header = self.tab_panel._tab_header
            ...
            # Устанавливаем фон для кнопок-вкладок
            for tab in self.tab_panel.tab_list:
                tab.background_normal = ''
                tab.background_down = ''
                tab.background_color = tab_bg

        # Привязываем обновление цвета при смене вкладки
    ...
    def _draw_tab_separators(self, separator_color):
        ...
        for i, tab in enumerate(tab_list):
            # НЕ ОЧИЩАЕМ canvas.before полностью, только удаляем старые линии
            # Сохраняем ссылку на линию если она есть
            ...
            # Рисуем разделитель справа от каждой вкладки (кроме последней)
            if i < len(tab_list) - 1:
    ...
    def _create_practice_tab(self):
        ...
        # ===== ИНТЕРАКТИВНЫЙ РЕДАКТОР С ПОЛЯМИ =====
        template = self.lesson_manager.get_lesson_template(lesson, lang)

        if template:
            # InteractiveCodeWidget сам является BoxLayout с внутренним ScrollView.
            # Добавляем его напрямую в таб — лишняя обёртка только мешает жестам.
            ...
            # Если шаблона нет, показываем обычный редактор в ScrollView
    ...
    def _update_code_from_editor(self):
        ...
        if hasattr(self.practice_editor, 'get_user_code'):
            ...
            print(f"_update_code_from_editor: '{self.user_code}'")
    ...
    def _create_task_tab(self):
        ...
        theme = ThemeManager.get_theme()
        lang = self.app.current_language
        task_text = self.lesson_manager.get_lesson_task(self.lesson, lang)
    ...
    def _on_code_change(self, instance, value):
        ...
        print(f"_on_code_change called, value type: {type(value)}")
        # Для InteractiveCodeWidget используем get_user_code()
        if hasattr(self.practice_editor, 'get_user_code'):
            self.user_code = self.practice_editor.get_user_code()
            print(f"user_code from get_user_code: '{self.user_code}'")
        else:
            self.user_code = value
            print(f"user_code from value: '{self.user_code}'")
        # Сохраняем код автоматически
        self.lesson_manager.save_lesson_code(self.lesson.get('id', 0), self.user_code)
    ...
    def _run_code(self, instance):
        ...
        # Используем существующий CodeExecutor из приложения
    ...
    def _check_task(self, instance):
        """Проверяет выполнение задания"""
        # Принудительно обновляем код перед проверкой
        if hasattr(self.practice_editor, 'get_user_code'):
            self.user_code = self.practice_editor.get_user_code()
            print(f"_check_task: user_code = '{self.user_code}'")

        if not self.user_code.strip():
            self.app.show_result_popup("Нет кода для проверки. Заполните поля.")
            return
        ...
        self.check_btn.text = "... " + self.app.tr.get('checking', 'Checking')

        def do_check():
            print("=== USER CODE ===")
            print(repr(self.user_code))
            print("=================")
            result = self.lesson_manager.check_lesson(
                self.lesson,
                self.user_code,
                self.app.current_language
            )

            Clock.schedule_once(lambda dt: self._show_check_result(result), 0)
    ...
    def _show_check_result(self, result):
        ...
        self.check_btn.text = "✓ " + self.app.tr.get('check_task', 'Check')

        theme = ThemeManager.get_theme()

        if result.get('success'):
            self.app.show_result_popup(
                f"✓ {result.get('message', 'Task completed!')}",
                success=True
            )
            # Если задание выполнено, подсвечиваем кнопку Complete
            if not self.complete_btn.disabled:
                self.complete_btn.background_color = theme.get('btn_success_bg', (0.2, 0.5, 0.2, 1))
        else:
            self.app.show_result_popup(
                f"✗ {result.get('message', 'Task failed')}",
                success=False
            )
    ...
    def _actual_mark_completed(self):
        """Фактически отмечает урок как пройденный"""
        lesson_id = self.lesson.get('id', 0)

        success = self.lesson_manager.mark_lesson_completed(lesson_id, self.user_code)
    ...
    def _offer_next_lesson(self):
        ...
        # Создаём диалог предложения
        content = BoxLayout(orientation='vertical', padding=dp(15), spacing=dp(15))

        msg_label = Label(
            text=tr.get('next_lesson_offer', 'Great job! Do you want to continue to the next lesson?'),
            font_size=dp(13),
            color=theme['text_color'],
            halign='center',
            size_hint_y=None,
    ...
    def _open_next_lesson(self, lesson):
        ...
        new_view = LessonView(self.app, lesson, self.lesson_manager)
