...
from ide_core import SettingsManager, ThemeManager, SyntaxStyleManager, TRANSLATIONS
...
from ide_core import ThemeManager
from utils.vibration_manager import VibrationManager
    ...
    def _on_language_select(self, lang_code):
        ...
        if lang_code != self.app.current_language:
            ...
            try:
                ...
                with open(user_data_path('language.txt'), 'w') as f:
                    f.write(lang_code)
            except:
                pass
...
class SyntaxHighlightMenu:
    ...
    def show(self, parent_button):
        ...
        # ========== ДОБАВИТЬ ОБЁРТКУ КНОПОК В СПИСКЕ СТИЛЕЙ ==========
        if hasattr(self.app, 'wrap_widget_buttons'):
            for child in styles_list.children:
                self.app.wrap_widget_buttons(child)
...
class SettingsMenu:
    ...
    def _open_vibration_submenu(self, parent_button):
        ...
        if not hasattr(self, '_vibration_menu') or self._vibration_menu is None:
            self._vibration_menu = VibrationSettingsMenu(self.app)

        self._vibration_menu.show(parent_button)
...
class VibrationSettingsMenu:
    """Подменю настроек вибрации"""
    ...
    def show(self, parent_button):
        ...
        # Получаем состояние вибрации
        try:
            current_enabled = VibrationManager.is_enabled()
        except:
            current_enabled = True

        # Получаем локализованные названия (короткие)
        app = App.get_running_app()
        tr = app.tr if app else {'get': lambda x, y: 'Vibrate'}

        # Короткие названия для кнопок
        on_text = tr.get('vibration_on', 'On') if hasattr(tr, 'get') else 'On'
        off_text = tr.get('vibration_off', 'Off') if hasattr(tr, 'get') else 'Off'
        ...
        # Текст с галочкой если активно
        display_text = f"✓ {on_text}" if current_enabled else f"    {on_text}"
        lbl1 = Label(
            text=display_text,
            color=theme['text_color'],
            font_size=dp(15),
        ...
        # Фон кнопки
        box1.canvas.before.clear()
        with box1.canvas.before:
            Color(*btn_bg)
            Rectangle(pos=box1.pos, size=box1.size)
            Color(btn_bg[0] + 0.08, btn_bg[1] + 0.08, btn_bg[2] + 0.08, 1)
            Line(rectangle=(box1.pos[0], box1.pos[1], box1.size[0], box1.size[1]), width=dp(0.5))

        # Обновляем фон при изменении позиции
        box1.bind(
            pos=lambda inst, val, bg=btn_bg: self._update_btn_bg(inst, bg),
            size=lambda inst, val, bg=btn_bg: self._update_btn_bg(inst, bg)
        )
        box1.bind(on_release=lambda x: self._toggle_vibration(True))
        self._dropdown.add_widget(box1)
        ...
        # Текст с галочкой если активно
        display_text2 = f"✓ {off_text}" if not current_enabled else f"    {off_text}"
        lbl2 = Label(
            text=display_text2,
            color=theme['text_color'],
            font_size=dp(15),
        ...
        box2.add_widget(icon2)
        box2.add_widget(lbl2)

        # Фон кнопки
        box2.canvas.before.clear()
        with box2.canvas.before:
            Color(*btn_bg)
            Rectangle(pos=box2.pos, size=box2.size)
            Color(btn_bg[0] + 0.08, btn_bg[1] + 0.08, btn_bg[2] + 0.08, 1)
            Line(rectangle=(box2.pos[0], box2.pos[1], box2.size[0], box2.size[1]), width=dp(0.5))
        ...
        box2.bind(on_release=lambda x: self._toggle_vibration(False))
        self._dropdown.add_widget(box2)
    ...
    def _toggle_vibration(self, enabled):
        ...
        try:
            app = App.get_running_app()
            if app and hasattr(app, 'show_result_popup'):
                tr = app.tr if app else {}
                msg = tr.get('vibration_on', 'Vibration ON') if enabled else tr.get('vibration_off', 'Vibration OFF')
                app.show_result_popup(f"✓ {msg}")
        except:
            pass

        # Короткая вибрация для обратной связи (если включена)
        if enabled:
            try:
                VibrationManager.vibrate(0.03)
            except:
                pass
